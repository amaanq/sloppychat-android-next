...
class ImagePackRepository(
    ...
    suspend fun getRoomPack(room: BaseRoom, stateKey: String): ResolvedImagePack? {
        ...
        // getStateEventRaw hands back the whole event, not just its content
        val content = lenientJson.parseToJsonElement(json).jsonObject["content"]?.toString() ?: json
        val pack = ImagePackParser.parsePackContent(content).getOrNull() ?: return null
        return ResolvedImagePack(
            pack = pack,
    ...
    suspend fun getEmoteRoomIds(): Map<RoomId, Set<String>> {
        ...
        Timber.d("ImagePacks: Found ${rooms.size} emote rooms")
        return rooms
    }
    ...
    suspend fun setRoomPack(room: BaseRoom, stateKey: String, pack: ImagePack): Result<Unit> {
        val json = ImagePackParser.serializePackContent(pack)
        return room.sendRawState(EVENT_TYPE_ROOM_EMOTES, stateKey, json).map { }
    }
