...
class ScRoomProfilePresenterTest {
    ...
    @Test
    fun `present - initial state is prefilled from the own member`() = runTest {
        val presenter = createPresenter()
        presenter.test {
            val loadedState = consumeItemsUntilPredicate { it.displayName == "Nick" }.last()
            assertThat(loadedState.ownUserId).isEqualTo(A_SESSION_ID)
            assertThat(loadedState.avatarUrl).isEqualTo(AN_AVATAR_URL)
            assertThat(loadedState.saveButtonEnabled).isFalse()
            assertThat(loadedState.avatarActions).containsExactly(
    ...
    @Test
    fun `present - save failure is exposed as failure state`() = runTest {
        val room = aRoom(
            sendRawState = { _, _, _ -> Result.failure(RuntimeException("nope")) },
        )
        val presenter = createPresenter(room = room)
        presenter.test {
            val loadedState = consumeItemsUntilPredicate { it.displayName == "Nick" }.last()
            ...
            val editedState = consumeItemsUntilPredicate { it.displayName == "New Nick" }.last()
            editedState.eventSink(ScRoomProfileEvent.Save)
            consumeItemsUntilPredicate { it.saveAction.isFailure() }
        }
    }
}
