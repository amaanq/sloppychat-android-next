...
class ScRoomProfilePresenter(
    ...
    private fun CoroutineScope.saveChanges(
        ...
        suspend {
            ...
            val existing = room.getRawState(EVENT_TYPE_ROOM_MEMBER, ownUserId).getOrThrow()
                ?: error("No membership event found for $ownUserId")
            val content = lenientJson.parseToJsonElement(existing).jsonObject.toMutableMap()
